...
##' @param LDresult A character value or a dataframe. As a dataframe, the data of linkage disequilibrium (LD) consisting of the maker name of other SNPs in plotting Region and 
...
##' @param geno A dataframe for genotype data. If we want to use own LD pattern, the package will automatically call "genetics" package to calculated LD. 
##' @param isRecomb a logical value indicating whether plots the recombination information
##' @param Recomb A dataframe contains the SNPs name and the recombination information, of which colnames are "MkName" and "recomb", respectively.
...
##' @param ref  the name of the reference genome. There are three different genome: 
##'             the widespread commercial breed: "Duroc" (Sscrofa10.2) or Chinese indigenous breed: Wzs (Wuzhishan) or other species: "Other".
##' @param Map The Map information for other species (used only if ref is specified for "Other")
##' @param Gff The reference genome for other species not pig  (used only if ref is specified for "Other")
...
plotRegion <- function(pval,plotChr,plotRegion,geneRegion,isLD=TRUE,LDresult,geno,isRecomb=TRUE,Recomb,isAnno=TRUE,main="",ref='Wzs',Map,Gff,xlab) {

  # mapping P-value to reference genome
  if(ref=="Other") pval <- merge(Map,pval,by="MkName") else pval <- merge(get(paste(ref,"Mk",sep="")),pval,by="MkName")
  ...
  if(isLD){
    if(missing(LDresult)) {
      ...
      LDresult <- LDresult[LDresult$SNP_A==HstSNP,][,c('SNP_B','R2')]
      pval<- merge(pval,LDresult,by.x='MkName',by.y='SNP_B',all.x=T)
    }
  }
  ...
  pval <- pval[order(pval$Position),] 
  if(ref=="Other") gff <- Gff else gff <- get(paste(ref,'Gff',sep=''))
